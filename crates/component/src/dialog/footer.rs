...
impl RenderOnce for DialogClose {
    fn render(self, window: &mut Window, cx: &mut App) -> impl IntoElement {
        ...
        div()
            .size_full()
            .id("dialog-close-wrapper")
            .track_focus(&focus_handle)
            .on_mouse_down(MouseButton::Left, {
                let focus_handle = focus_handle.clone();
                move |_, window, cx| {
                    // Explicitly focus this element so that the action dispatched by
                    // the wrapped element routes through the Dialog's on_action
                    // handler. Button children call window.prevent_default() which
                    // suppresses automatic focus, so we must focus explicitly here.
