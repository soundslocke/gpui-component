...
#[derive(Clone)]
enum SidebarMenuRow {
    /// Boxed to keep a menu's `Vec` of rows from sizing every entry, including
    /// the pointer-sized `Custom` ones, to a whole item.
...
impl SidebarMenu {
    ...
    pub fn child(mut self, child: impl Into<SidebarMenuItem>) -> Self {
        self.items
            .push(SidebarMenuRow::Item(Box::new(child.into())));
        self
    }
    ...
    pub fn children(
        ...
        self.items = children
            .into_iter()
            .map(|child| SidebarMenuRow::Item(Box::new(child.into())))
            .collect();
        self
    }
